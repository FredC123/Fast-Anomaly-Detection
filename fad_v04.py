...
def Get_SandD(td_local,acc=6,
    ...
    while j < LS-windowsizeright :
        ...
        if dfSin["spike"][j-1] >= 1 or dfSin["spike"][j-1] <= -1:
            dfSin["spike"][j] = 0
            dfSin["Reason"][j] = "set to 0 due to previous"
        elif dfSin[KPI][j] > dfSin["Regr_value"][j] + acc*dfSin["Std"][j] and not between(dfSin[KPI][j-1],
                                                                                          dfSin[KPI][j],
                                                                                          dfSin[KPI][j+1]):
            dfSin["spike"][j] = 1
            dfSin["Reason"][j] = "above acc-times above regression value"
        elif dfSin[KPI][j] < dfSin["Regr_value"][j] - acc*dfSin["Std"][j] and not between(dfSin[KPI][j-1],
                                                                                          dfSin[KPI][j],
                                                                                          dfSin[KPI][j+1]):
            dfSin["spike"][j] = -1
            dfSin["Reason"][j] = "above acc-times below regression value"
        elif dfSin["Std"][j] > P1 * dfSin["Std"][j-1]:
            dfSin["spike"][j] =  (dfSin[KPI][j] - dfSin["Regr_value"][j])/abs(dfSin[KPI][j] - dfSin["Regr_value"][j]) * P1 # +1 or -1 P1
            dfSin["Reason"][j] = "Slope leftpoint x sd < Slope" #"Stdj  P1times Stdj1"
        elif abs(dfSin["Slope"][j-windowsizeleft]) * sd < abs(dfSin["Slope"][j]):
            dfSin["spike"][j] = 1
            dfSin["Reason"][j] = "Slope leftpoint x sd < Slope"
        elif abs(dfSin["Slope"][j+windowsizeright]) * sd < abs(dfSin["Slope"][j]):
            dfSin["spike"][j] = -1
            dfSin["Reason"][j] = "Slope rightpoint x sd < Slope"
        else:
            pass
        j += 1
